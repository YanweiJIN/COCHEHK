...
class Model(object):
    ...
    def _build_projector(self, hparams, field=None):
        ...
        if field == "src":
            if hparams.projector_type == "linear":
                ...
                projector = tf.layers.Dense(self.src_feature_size, use_bias=True, activation=tf.math.sigmoid)
        elif field == "tgt":
            if hparams.projector_type == "linear":
                projector = tf.layers.Dense(self.tgt_feature_size, use_bias=True)
            elif hparams.projector_type == "sigmoid":
                projector = tf.layers.Dense(self.tgt_feature_size, use_bias=True, activation=tf.math.sigmoid)
        else:
    ...
    def set_mode_phase(self, hparams):
        ...
        if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
            ...
            # backward phase
            self.src_ref_given = tf.placeholder(self.dtype, shape=(None, self.length, self.src_feature_size))
            self.tgt_ref_given = tf.placeholder(self.dtype, shape=(None, self.length, self.tgt_feature_size))
            self.src2tgt_output_given = tf.placeholder(self.dtype,
                                                       shape=(None, self.length, self.tgt_feature_size))
            self.tgt2src_output_given = tf.placeholder(self.dtype,
                                                       shape=(None, self.length, self.src_feature_size))
            self.src_seq_len_given = tf.placeholder(tf.int32, shape=(None))
            self.tgt_seq_len_given = tf.placeholder(tf.int32, shape=(None))
