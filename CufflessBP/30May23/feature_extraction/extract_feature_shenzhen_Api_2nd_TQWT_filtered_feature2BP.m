...
% setting
disp_original_BP_ECG_PPG=1; % 0 not display, 1 display
...
subdir_storedata='/TQWT_filtered_feature_data_Api_13th_feature2BP_overlap';
...
%         linkaxes(sub,'x');
        ...
        for(k=[1:1:max(statistic)])
            ...
            RI(k,1)=PPG(systolicIndex_PPG(1,k),1)-PPG(dicroticIndex_PPG(1,k),1);
            Systolic_timespan(k,1)=notchIndex_PPG(1,k)-footIndex_PPG(1,k);
        ...
        feature_person=horzcat(PTTp,PTTf,RI,Systolic_timespan,Up_time,Systolic_volume,Diastolic_volume,HR,SBP,DBP,MBP);
        ...
        vars={'PTTp', 'PTTf', 'RI', 'Systolic_timespan', 'Up_time', 'Systolic_volume', 'Diastolic_volume', 'HR',...
